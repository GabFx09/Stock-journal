...
function doGet(e) {
  try {
    ...
    if (action === 'getData') {
      ...
      const { profile, saldoAwal } = getProfileData(ss);
      return ok({
...
function doPost(e) {
  try {
    ...
    if (action === 'syncAll') {
      ...
      // Profil & Saldo
      if (payload.profile) saveProfileData(ss, payload.profile);
      if (payload.saldoAwal != null) saveSaldoAwal(ss, payload.saldoAwal);
...
function saveProfileData(ss, profile) {
  ...
  // Hapus baris profil lama (kecuali baris saldoAwal)
  const rows = sheet.getDataRange().getValues();
  for (let i = rows.length - 1; i >= 1; i--) {
    if (String(rows[i][0]) !== 'saldoAwal') sheet.deleteRow(i + 1);
  }
  Object.entries(profile || {}).forEach(([k, v]) => sheet.appendRow([k, v]));
}

function saveSaldoAwal(ss, amount) {
  const sheet = getOrCreate(ss, 'Profil', ['key','value']);
  const rows = sheet.getDataRange().getValues();
  for (let i = 1; i < rows.length; i++) {
    if (String(rows[i][0]) === 'saldoAwal') {
      sheet.getRange(i + 1, 2).setValue(amount);
      return;
    }
  }
  sheet.appendRow(['saldoAwal', amount]);
}

function getProfileData(ss) {
  const sheet = ss.getSheetByName('Profil');
  if (!sheet || sheet.getLastRow() <= 1) return { profile: {}, saldoAwal: 0 };
  const rows = sheet.getDataRange().getValues().slice(1);
  const profile = {};
  let saldoAwal = 0;
  rows.forEach(r => {
    if (String(r[0]) === 'saldoAwal') saldoAwal = parseFloat(r[1]) || 0;
    else profile[String(r[0])] = r[1];
  });
  return { profile, saldoAwal };
}
